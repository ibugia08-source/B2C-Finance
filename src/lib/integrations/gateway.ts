...
/**
 * Entrada de webhook do gateway — pela caixa genérica de `integrations/inbox`
 * (F5.3), que carrega as lições da F4.8: assinatura do corpo CRU em tempo
 * constante, idempotência pela unique (source, eventId), "repetido" só com
 * DESFECHO, processamento sob o dono da conta e falha reprocessável.
 */
export async function receberEventoDoGateway(
  corpo: string,
  assinatura: string | null
): Promise<ResultadoDoGateway> {
  const { receberNaCaixa } = await import("@/lib/integrations/inbox");
  return receberNaCaixa({
    fonte: FONTE_GATEWAY,
    segredo: segredoDoGateway(),
    erroDeConfiguracao: "Integração não configurada. Defina GATEWAY_WEBHOOK_SECRET.",
    corpo,
    assinatura,
    processar,
  });
